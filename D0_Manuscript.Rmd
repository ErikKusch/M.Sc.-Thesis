...
col.qsoil <- got(n = 4, alpha = 1, begin = 0, end = 1, direction = 1, option = "white_walkers")
col.mem <- got(n = 4, alpha = 1, begin = 0, end = 1, direction = 1, option = "greyjoy")
  VarHold <- Variable
  ...
  return(list(leg,Output,dim(plot_df)[1]/10))
}
```
...
\begin{enumerate}
\item \qsoila memory length (identified as cumulative lags according to \aic values) exhibits low values (short memory) in the region of the Pyrenees with a noticeable shift towards slightly longer memory lengths when moving to dryland regions (particularly across Spain).
...
\begin{enumerate}
\item \qsoila memory length (identified as cumulative lags according to \aic values) exhibits lower values (shorter memory) in the north-eastern dryland regions as well as the south-western tropical regions.
\item \ndvil memory depicts the same pattern as the vegetation sensitivity index developed by Seddon et al\cite{Seddon2016} with the north-eastern dryland region being characterised by strong intrinsic memory effects. Notice the negligible coverage of negative \ndvil memory effects as well as their minute absolute values when compared to positive \ndvil memory effect values.
\item \qsoila memory effects are mostly positive across the entire Caatinga range with the highest absolute values being found in the north-eastern dryland region. Take note that the maximum \qsoila memory coefficient is larger than the absolute of the corresponding minimum value. Additionally, there are some changes in \qsoil memory effect sign between \qsoila and \qsoilb, and \qsoilc and \qsoild with negative \qsoil effects being identified around the region of Brasilia in the latter two layers.
\item \tair memory patterns follow a pattern that is close to the inverse of the \qsoila pattern with large negative values in the north-eastern dryland region. Take note that the absolute of the minimum \tair memory coefficient is larger than the corresponding maximum value.
\end{enumerate}
...
Again, only \qsoila results are presented here. The hierarchy of median vegetation memory coefficients remains unaltered when considering other \qsoil layers (see tables \ref{tab:CAU2}, \ref{tab:CAU3}, \ref{tab:CAU4}).
...
```{r CAVarPar, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Soil Layer 1', 'Soil Layer 2', 'Soil Layer 3', 'Soil Layer 4'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Variance Partitioning (Caatinga; Qsoil1) -} Variance of \\ndvi anomalies explained by (a) full models of intrinsic and extrinsic memory, (b) intrinsic memory, (c) shared variance, and (d) extrinsic memory. A representation of how these were calculated can be retrieved in figure \\ref{fig:VarPar}. Figure established via \\ref{rVarParRes}. \\label{fig:CAVarPar1}", fig.scap="Variance Partitioning (Caatinga; Qsoil1)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
\item \tair memory patterns follow a pattern that is close to the inverse of the \tair pattern with large negative values across all of Australia. Take note that the absolute of the minimum \tair memory coefficient is larger than the corresponding maximum value.
\end{enumerate}
...
Again, only \qsoila results are presented here. The hierarchy of median vegetation memory coefficients remains unaltered when considering other \qsoil layers (see tables \ref{tab:AUU2}, \ref{tab:AUU3}, \ref{tab:AUU4}).
...
```{r AUVarPar, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Soil Layer 1', 'Soil Layer 2', 'Soil Layer 3', 'Soil Layer 4'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Variance Partitioning (Australia; Qsoil1) -} Variance of \\ndvi anomalies explained by (a) full models of intrinsic and extrinsic memory, (b) intrinsic memory, (c) shared variance, and (d) extrinsic memory. A representation of how these were calculated can be retrieved in figure \\ref{fig:VarPar}. Figure established via \\ref{rVarParRes}. \\label{fig:AUVarPar1}", fig.scap="Variance Partitioning (Australia; Qsoil1)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
#### Iberian Region
\paragraph*{Intrinsic Memory and Air Temperature}  \hspace{.5cm}

Sensitivity relationships of \ndvil and \tair are depicted alongside raw data in figure \ref{fig:IBPatCausRes}.
\vspace{-.3cm}
...
Sensitivity relationships of \qsoila memory length and \qsoila are depicted alongside raw data in figure \ref{fig:IBPatCausRes1}.
\vspace{-.3cm}
...
Sensitivity relationships of \qsoilb memory length and \qsoilb are depicted alongside raw data in figure \ref{fig:IBPatCausRes2}.
\vspace{-.3cm}
...
Sensitivity relationships of \qsoilc memory length and \qsoilc are depicted alongside raw data in figure \ref{fig:IBPatCausRes3}.
\vspace{-.3cm}
...
Sensitivity relationships of \qsoild memory length and \qsoild are depicted alongside raw data in figure \ref{fig:IBPatCausRes4}.
\vspace{-.3cm}
...
The sensitivity of \qsoil vegetation memory to mean \qsoil layer data diminishes through \qsoil layers (from shallow to deep layers). This effect has been identified as statistically significant according to an \anova at $p = `r IBANOVA[[2]][1,5]`$ (established via \ref{rPatCausAnova}).

The correlation of \qsoil vegetation memory length to mean \qsoil layer data starts of negative for \qsoila and \qsoilb (diminishing when going from \qsoila to \qsoilb) and flips to being positive for \qsoilc and \qsoild (intensifying from \qsoilc to \qsoild). This effect has been identified as statistically significant according to an \anova at $p = `r IBANOVA[[1]][1,5]`$  (established via \ref{rPatCausAnova}).

\clearpage

#### Caatinga
\paragraph*{Intrinsic Memory and Air Temperature}  \hspace{.5cm}
Sensitivity relationships of \ndvil and \tair are depicted alongside raw data in figure \ref{fig:CAPatCausRes}.
\vspace{-.3cm}
...
\paragraph*{Soil Moisture (0-7cm)}  \hspace{.5cm}
Sensitivity relationships of \qsoila memory length and \qsoila are depicted alongside raw data in figure \ref{fig:CAPatCausRes1}.
\vspace{-.3cm}
...
\paragraph*{Soil Moisture (7-28cm)}  \hspace{.5cm}
Sensitivity relationships of \qsoilb memory length and \qsoilb are depicted alongside raw data in figure \ref{fig:CAPatCausRes2}.
\vspace{-.3cm}
...
\paragraph*{Soil Moisture (28-100cm)}  \hspace{.5cm}
Sensitivity relationships of \qsoilc memory length and \qsoilc are depicted alongside raw data in figure \ref{fig:CAPatCausRes3}.
\vspace{-.3cm}
...
\paragraph*{Soil Moisture (100-255cm)}  \hspace{.5cm}
Sensitivity relationships of \qsoild memory length and \qsoild are depicted alongside raw data in figure \ref{fig:CAPatCausRes4}.
\vspace{-.3cm}
...
Linear regression coefficients of vegetation memory sensitivity relationships across the Caatinga are presented in table \ref{tab:AUPatCauseTab}. Almost all vegetation memory coefficients establish themselves in negative correlations with respective mean environmental values. \qsoila, \qsoilb, \qsoilc, and \ndvil are linked especially strongly to environmental mean drivers.
...
The sensitivity of \qsoil vegetation memory to mean \qsoil layer data fluctuates across the soil layer range whilst diminishing from \qsoila to \qsoilb. These differences have been been identified as statistically significant according to an \anova at $p = `r CAANOVA[[2]][1,5]`$ (established via \ref{rPatCausAnova}).

The correlation of \qsoil vegetation memory length to mean \qsoil layer follows no identifiable linear pattern. Differences in linear regressions have been identified as statistically significant according to an \anova at $p = `r CAANOVA[[1]][1,5]`$  (established via \ref{rPatCausAnova}).

\clearpage

#### Australia
\paragraph*{Intrinsic Memory and Air Temperature}  \hspace{.5cm}
Sensitivity relationships of \ndvil and \tair are depicted alongside raw data in figure \ref{fig:AUPatCausRes}.
\vspace{-.3cm}
...
\paragraph*{Soil Moisture (0-7cm)}  \hspace{.5cm}
Sensitivity relationships of \qsoila memory length and \qsoila are depicted alongside raw data in figure \ref{fig:AUPatCausRes1}.
\vspace{-.3cm}
...
\paragraph*{Soil Moisture (7-28cm)}  \hspace{.5cm}
Sensitivity relationships of \qsoilb memory length and \qsoilb are depicted alongside raw data in figure \ref{fig:AUPatCausRes2}.
\vspace{-.3cm}
...
\paragraph*{Soil Moisture (28-100cm)}  \hspace{.5cm}
Sensitivity relationships of \qsoilc memory length and \qsoilc are depicted alongside raw data in figure \ref{fig:AUPatCausRes3}.
\vspace{-.3cm}
...
\paragraph*{Soil Moisture (100-255cm)}  \hspace{.5cm}
Sensitivity relationships of \qsoild memory length and \qsoild are depicted alongside raw data in figure \ref{fig:AUPatCausRes4}.
\vspace{-.3cm}
...
The sensitivity of \qsoil vegetation memory to mean \qsoil layer data fluctuates across the soil layer range with \qsoild coefficients' relation to mean \qsoil drivers being strong and positive. These differences have been been identified as statistically significant according to an \anova at $p = `r AUANOVA[[2]][1,5]`$ (established via \ref{rPatCausAnova}).

The correlation of \qsoil vegetation memory length to mean \qsoil layer follows no identifiable linear pattern. Differences in linear regressions have been identified as statistically significant according to an \anova at $p = `r AUANOVA[[1]][1,5]`$  (established via \ref{rPatCausAnova}).
...
\begin{enumerate}
\item \textbf{\qsoila memory length} (identified as cumulative lags according to \aic values) exhibits \textbf{shorter} values \textbf{in drylands} when compared to adjacent non-dryland regions.
...
\item \textbf{\qsoila memory} effects are \textbf{strong and positive across dryland regions}, largely coinciding with \ndvil effects.
\item \textbf{\tair memory} patterns are usually \textbf{strong and negative in dryland regions}.
\end{enumerate}

Across the Iberian Region, \qsoila memory length is shortes for regions of negative \qsoila memory effect impacts whilst short \qsoila memory overlaps with regions of high positive \qsoila memory effects across the Caatinga. Across Australia, no such overlap of patterns can be found.
...
\item \textbf{\tair} memory coefficients are \textbf{weakly negatively correlated} to mean \tair records.
\item \textbf{\qsoil} memory coefficients are \textbf{overwhelmingly correlated strongly negatively} to mean \qsoil records.
...
```{r IBCOMPAD1, echo = FALSE, fig.width = 17, fig.height = 9.5, fig.cap="\\textbf{COMPADRE FSC-1 and Vegetation Memory (Iberian Region) -} Linear regression coefficients of COMPADRE FSC-1 and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:IBCOMPAD1}", fig.scap="COMPADRE FSC-1 and Vegetation Memory (Iberian Region)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
...
```{r IBCOMPAD2, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE FSC-2 and Vegetation Memory (Iberian Region) -} Linear regression coefficients of COMPADRE FSC-2 and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:IBCOMPAD2}", fig.scap="COMPADRE FSC-2 and Vegetation Memory (Iberian Region)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
...
```{r IBCOMPAD3, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE Period of Oscillation and Vegetation Memory (Iberian Region) -} Linear regression coefficients of COMPADRE Period of Oscillation and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:IBCOMPAD3}", fig.scap="COMPADRE Period of Oscillation and Vegetation Memory (Iberian Region)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg3 <- Comres(Region = "Iberian Region", Variable = "Pi")
```

Linear relationships of \cpi and vegetation memory coefficients across the Iberian region are built on `r leg3[[3]]` data points each. Only intercepts of \qsoil memory length and \ndvil regressions are statistically significant. Regression slopes, on the other hand, are only significant for \qsoilb, \qsoilc, and \qsoild.
...
```{r IBCOMPAD4, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE Damping Ratio and Vegetation Memory (Iberian Region) -} Linear regression coefficients of COMPADRE Damping Ratio and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:IBCOMPAD4}", fig.scap="COMPADRE Damping Ratio and Vegetation Memory (Iberian Region)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg4 <- Comres(Region = "Iberian Region", Variable = "Rho")
```

Linear relationships of \crho and vegetation memory coefficients across the Iberian region are built on `r leg4[[3]]` data points each. Intercepts of \ndvil, \qsoila, \qsoilb, \qsoilc, \qsoila memory length, and \qsoilb memory length are statistically significant. Regression slopes, on the other hand, are not statistically significant for any of the regressions.
...
```{r IBCOMPAD5, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE Reactivity and Vegetation Memory (Iberian Region) -} Linear regression coefficients of COMPADRE Reactivity and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:IBCOMPAD5}", fig.scap="COMPADRE Reactivity and Vegetation Memory (Iberian Region)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg5 <- Comres(Region = "Iberian Region", Variable = "Reactivity")
```

Linear relationships of reactivity and vegetation memory coefficients across the Iberian region are built on `r leg5[[3]]` data points each. Whilst \COMPADRE reactivity is not linked statistically significantly to \qsoil memory lengths, relationships of all other vegetation memory coefficients and \COMPADRE reactivity are. 

\ndvil and \qsoil memory effects decrease with increasing \COMPADRE reactivity. This effect diminishes throughout the \qsoil layers but stays statistically significant. \tair memory effects increase with increasing \COMPADRE reactivity.
...
Relationships of vegetation memory coefficients and \fsca records across the Caatinga are represented in figure \ref{fig:CACOMPAD1}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:CACOMPADTab1}.

```{r CACOMPAD1, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE FSC-1 and Vegetation Memory (Caatinga) -} Linear regression coefficients of COMPADRE FSC-1 and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:CACOMPAD1}", fig.scap="COMPADRE FSC-1 and Vegetation Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg1 <- Comres(Region = "Caatinga", Variable = "FSC-1")
```

Linear relationships of \fsca and vegetation memory coefficients across the Caatinga are built on `r leg1[[3]]` data points each. Almost all linear regressions coefficients are not statistically significant.
...
Relationships of vegetation memory coefficients and \fscb records across the Caatinga are represented in figure \ref{fig:CACOMPAD2}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:CACOMPADTab2}.

```{r CACOMPAD2, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE FSC-2 and Vegetation Memory (Caatinga) -} Linear regression coefficients of COMPADRE FSC-2 and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:CACOMPAD2}", fig.scap="COMPADRE FSC-2 and Vegetation Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg2 <- Comres(Region = "Caatinga", Variable = "FSC-2")
```

Linear relationships of \fscb and vegetation memory coefficients across the Caatinga are built on `r leg2[[3]]` data points each. Almost all linear regressions coefficients are not statistically significant.
...
Relationships of vegetation memory coefficients and \cpi records across the Caatinga are represented in figure \ref{fig:CACOMPAD3}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:CACOMPADTab3}.

```{r CACOMPAD3, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE Period of Oscillation and Vegetation Memory (Caatinga) -} Linear regression coefficients of COMPADRE Period of Oscillation and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:CACOMPAD3}", fig.scap="COMPADRE Period of Oscillation and Vegetation Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg3 <- Comres(Region = "Caatinga", Variable = "Pi")
```

Linear relationships of \cpi and vegetation memory coefficients across the Caatinga are built on `r leg3[[3]]` data points each. Again, almost all linear regressions coefficients are not statistically significant.
...
Relationships of vegetation memory coefficients and \crho records across the Caatinga are represented in figure \ref{fig:CACOMPAD4}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:CACOMPADTab4}.

```{r CACOMPAD4, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE Damping Ratio and Vegetation Memory (Caatinga) -} Linear regression coefficients of COMPADRE Damping Ratio and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:CACOMPAD4}", fig.scap="COMPADRE Damping Ratio and Vegetation Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg4 <- Comres(Region = "Caatinga", Variable = "Rho")
```

Linear relationships of \crho and vegetation memory coefficients across the Caatinga are built on `r leg4[[3]]` data points each. Once more, almost all linear regressions coefficients are not statistically significant.
...
\paragraph*{Reactivity}  \hspace{.5cm}
Relationships of vegetation memory coefficients and reactivity records across the Caatinga are represented in figure \ref{fig:CACOMPAD5}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:CACOMPADTab5}.

```{r CACOMPAD5, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE Reactivity and Vegetation Memory (Caatinga) -} Linear regression coefficients of COMPADRE Reactivity and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:CACOMPAD5}", fig.scap="COMPADRE Reactivity and Vegetation Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg5 <- Comres(Region = "Caatinga", Variable = "Reactivity")
```

Linear relationships of reactivity and vegetation memory coefficients across the Caatinga are built on `r leg5[[3]]` data points each. Again, my analyses did not identify statistically significant relationships between \COMPADRE reactivity and vegetation memory coefficients.
...
Relationships of vegetation memory coefficients and \fsca records across Australia are represented in figure \ref{fig:AUCOMPAD1}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:AUCOMPADTab1}.

```{r AUCOMPAD1, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE FSC-1 and Vegetation Memory (Australia) -} Linear regression coefficients of COMPADRE FSC-1 and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:AUCOMPAD1}", fig.scap="COMPADRE FSC-1 and Vegetation Memory (Australia)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg1 <- Comres(Region = "Australia", Variable = "FSC-1")
```

Linear relationships of \fsca and vegetation memory coefficients across Australia are built on `r leg1[[3]]` data points each. Whilst all intercepts of these regressions are statistically significant, none of the regression slopes are.
...
Relationships of vegetation memory coefficients and \fscb records across Australia are represented in figure \ref{fig:AUCOMPAD2}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:AUCOMPADTab2}.

```{r AUCOMPAD2, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE FSC-2 and Vegetation Memory (Australia) -} Linear regression coefficients of COMPADRE FSC-2 and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:AUCOMPAD2}", fig.scap="COMPADRE FSC-2 and Vegetation Memory (Australia)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg2 <- Comres(Region = "Australia", Variable = "FSC-2")
```

Linear relationships of \fscb and vegetation memory coefficients across Australia are built on `r leg2[[3]]` data points each. Whilst almost all intercepts of these regressions are statistically significant, none of the regression slopes are.
...
Relationships of vegetation memory coefficients and \cpi records across Australia are represented in figure \ref{fig:AUCOMPAD3}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:AUCOMPADTab3}.

```{r AUCOMPAD3, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE Period of Oscillation and Vegetation Memory (Australia) -} Linear regression coefficients of COMPADRE Period of Oscillation and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:AUCOMPAD3}", fig.scap="COMPADRE Period of Oscillation and Vegetation Memory (Australia)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg3 <- Comres(Region = "Australia", Variable = "Pi")
```

Linear relationships of \cpi and vegetation memory coefficients across Australia are built on `r leg3[[3]]` data points each. Again, almost all intercepts of these regressions are statistically significant. However, only some linear regression slopes pertaining mostly to \qsoil memory effects and memory lengths are statistically significant.
...
Relationships of vegetation memory coefficients and \crho records across Australia are represented in figure \ref{fig:AUCOMPAD4}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:AUCOMPADTab4}.

```{r AUCOMPAD4, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE Damping Ratio and Vegetation Memory (Australia) -} Linear regression coefficients of COMPADRE Damping Ratio and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:AUCOMPAD4}", fig.scap="COMPADRE Damping Ratio and Vegetation Memory (Australia)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg4 <- Comres(Region = "Australia", Variable = "Rho")
```

Linear relationships of \crho and vegetation memory coefficients across Australia are built on `r leg4[[3]]` data points each. None of the delineated linear regression coefficients are statistically significant.
...
Relationships of vegetation memory coefficients and reactivity records across Australia are represented in figure \ref{fig:AUCOMPAD5}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:AUCOMPADTab5}.

```{r AUCOMPAD5, echo = FALSE, fig.width = 17, fig.height = 10, fig.cap="\\textbf{COMPADRE Reactivity and Vegetation Memory (Australia) -} Linear regression coefficients of COMPADRE Reactivity and vegetation memory coefficients. Figure established via \\ref{rCOMRes}. \\label{fig:AUCOMPAD5}", fig.scap="COMPADRE Reactivity and Vegetation Memory (Australia)", cache = TRUE, cache.lazy=FALSE, out.extra='', fig.pos = 'h!'}
leg5 <- Comres(Region = "Australia", Variable = "Reactivity")
```

Linear relationships of reactivity and vegetation memory coefficients across Australia are built on `r leg5[[3]]` data points each. My analyses identified statistically significant relationships for all vegetation memory coefficients but \qsoil memory lengths with positive relationships between \ndvil as well as \qsoil memory effects and \COMPADRE reactivity. With the exception of \tair, all of these relationships are positive correlations.
...
Relationships of \ndvil vegetation memory coefficients and \vh, and \nmass records across the Iberian region are represented in figure \ref{fig:IBPFT1}. These relationships have been assessed through linear regressions. 
```{r IBPFT1, echo = FALSE, fig.width = 17, fig.height = 5.5, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 1, out.width='1\\linewidth', fig.cap="\\textbf{PFTs and Intrinsic Memory (Iberian Region) -} Intrinsic Memory (NDVI t-1, figure \\ref{fig:IBRVegMem1}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:IBPTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:IBPFT1}", fig.scap="PFTs and Intrinsic Memory (Iberian Region)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
IBPFT1 <- Tryres(Region = "Iberian Region", SoilLayer = 1, Variable = "t-1")
```

\vh is correlated with \ndvil memory effects as:
\begin{enumerate}
\item $Intercept = `r IBPFT1[[1]]`$ ($p_{Intercept} = `r IBPFT1[[2]]`$); $Slope = `r IBPFT1[[3]]`$ ($p_{Slope} = `r IBPFT1[[4]]`$) for untreated, geo-referenced data
\item $Intercept = `r IBPFT1[[5]]`$ ($p_{Intercept} = `r IBPFT1[[6]]`$); $Slope = `r IBPFT1[[7]]`$ ($p_{Slope} = `r IBPFT1[[8]]`$) for mapped mean trait data
\end{enumerate}

\nmass is correlated with \ndvil memory effects as:
\begin{enumerate}
\item $Intercept = `r IBPFT1[[9]]`$ ($p_{Intercept} = `r IBPFT1[[10]]`$); $Slope = `r IBPFT1[[11]]`$ ($p_{Slope} = `r IBPFT1[[12]]`$) for untreated, geo-referenced data
\item $Intercept = `r IBPFT1[[13]]`$ ($p_{Intercept} = `r IBPFT1[[14]]`$); $Slope = `r IBPFT1[[15]]`$ ($p_{Slope} = `r IBPFT1[[16]]`$) for mapped mean trait data
\end{enumerate}

\clearpage

\paragraph*{Air Temperature}  \hspace{.5cm}
Relationships of \tair vegetation memory coefficients and \vh, and \nmass records across the Iberian region are represented in figure \ref{fig:IBPFT2}. These relationships have been assessed through linear regressions. 
```{r IBPFT2, echo = FALSE, fig.width = 17, fig.height = 5.5, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 1, out.width='1\\linewidth', fig.cap="\\textbf{PFTs and Tair Memory (Iberian Region) -} Tair Memory (figure \\ref{fig:IBRVegMem1}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:IBPTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:IBPFT2}", fig.scap="PFTs and Tair Memory (Iberian Region)",  cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
IBPFT2 <- Tryres(Region = "Iberian Region", SoilLayer = 1, Variable = "Tair")
```
\vh is correlated with \tair memory effects as:
\begin{enumerate}
\item $Intercept = `r IBPFT2[[1]]`$ ($p_{Intercept} = `r IBPFT2[[2]]`$); $Slope = `r IBPFT2[[3]]`$ ($p_{Slope} = `r IBPFT2[[4]]`$) for untreated, geo-referenced data
\item $Intercept = `r IBPFT2[[5]]`$ ($p_{Intercept} = `r IBPFT2[[6]]`$); $Slope = `r IBPFT2[[7]]`$ ($p_{Slope} = `r IBPFT2[[8]]`$) for mapped mean trait data
\end{enumerate}

\nmass is correlated with \tair memory effects as:
\begin{enumerate}
\item $Intercept = `r IBPFT2[[9]]`$ ($p_{Intercept} = `r IBPFT2[[10]]`$); $Slope = `r IBPFT2[[11]]`$ ($p_{Slope} = `r IBPFT2[[12]]`$) for untreated, geo-referenced data
\item $Intercept = `r IBPFT2[[13]]`$ ($p_{Intercept} = `r IBPFT2[[14]]`$); $Slope = `r IBPFT2[[15]]`$ ($p_{Slope} = `r IBPFT2[[16]]`$) for mapped mean trait data
\end{enumerate}
...
\paragraph*{Soil Moisture (7-28cm)}  \hspace{.5cm}
Relationships of \qsoilb vegetation memory length, and coefficients and \vh, and \nmass records across the Iberian region are represented in figure \ref{fig:IBPFT4}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:IBPFTTab4}. 
```{r IBPFT4, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil2 Memory (Iberian Region) -} \\qsoilb Memory (figure \\ref{fig:IBRVegMem2}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:IBPTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:IBPFT4}", fig.scap="PFTs and Qsoil2 Memory (Iberian Region)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
IBPFT4 <- Tryres(Region = "Iberian Region", SoilLayer = 2, Variable = "Qsoil")
```
My analyses identified statistically significant relationships for all \qsoilb related vegetation memory coefficients with the exception of \vh and \qsoilb memory length. Whilst geo-referenced data records of \vh and \nmass establish themselves in positive correlations with \qsoilb memory effects, mapped mean trait values of \vh and \nmass are correlated negatively to \qsoilb related vegetation memory coefficients. \qsoilb memory length is correlated negatively to both \vh and \nmass.
...
\paragraph*{Soil Moisture (28-100cm)}  \hspace{.5cm}
Relationships of \qsoilc vegetation memory length, and coefficients and \vh, and \nmass records across the Iberian region are represented in figure \ref{fig:IBPFT5}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:IBPFTTab5}. 
```{r IBPFT5, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil3 Memory (Iberian Region) -} \\qsoilc Memory (figure \\ref{fig:IBRVegMem3}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:IBPTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:IBPFT5}", fig.scap="PFTs and Qsoil3 Memory (Iberian Region)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
IBPFT5 <- Tryres(Region = "Iberian Region", SoilLayer = 3, Variable = "Qsoil")
```
Linear regression analyses identified statistically significant relationships for all \qsoilc related vegetation memory coefficients with the exception of \vh and \qsoilc memory length. Whilst geo-referenced data records of \vh and \nmass establish themselves in positive correlations with \qsoilc memory effects, mapped mean trait values of \vh and \nmass are correlated negatively to \qsoilc related vegetation memory coefficients. \qsoilc memory length is correlated negatively to both \vh and \nmass.
...
\paragraph*{Soil Moisture (100-255cm)}  \hspace{.5cm}
Relationships of \qsoild vegetation memory length, and coefficients and \vh, and \nmass records across the Iberian region are represented in figure \ref{fig:IBPFT6}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:IBPFTTab6}. 
```{r IBPFT6, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil4 Memory (Iberian Region) -} \\qsoild Memory (figure \\ref{fig:IBRVegMem4}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:IBPTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:IBPFT6}", fig.scap="PFTs and Qsoil4 Memory (Iberian Region)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
IBPFT6 <- Tryres(Region = "Iberian Region", SoilLayer = 4, Variable = "Qsoil")
```
Linear regression analyses identified statistically significant relationships for all \qsoild related vegetation memory coefficients with the exception of geo-referenced \vh and \qsoilc memory effects, and memory length as well as geo-referenced \nmass and \qsoild memory lengths. Whilst geo-referenced data records of \vh and \nmass establish themselves in positive correlations with \qsoild related memory coefficients, mapped mean trait values of \vh and \nmass are correlated negatively to \qsoild related vegetation memory coefficients.
...
#### Caatinga
\paragraph*{Intrinsic Memory}  \hspace{.5cm}
Relationships of \ndvil vegetation memory coefficients and \vh, and \nmass records across the Caatinga are represented in figure \ref{fig:CAPFT1}. These relationships have been assessed through linear regressions. 
```{r CAPFT1, echo = FALSE, fig.width = 17, fig.height = 5.5, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 1, out.width='1\\linewidth', fig.cap="\\textbf{PFTs and Intrinsic Memory (Caatinga) -} Intrinsic Memory (NDVI t-1, figure \\ref{fig:CAVegMem1}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:CaatTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:CAPFT1}", fig.scap="PFTs and Intrinsic Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
CAPFT1 <- Tryres(Region = "Caatinga", SoilLayer = 1, Variable = "t-1")
```

Correlations of \vh with \ndvil memory effects are as follows:
\begin{enumerate}
\item Cannot be calculated for untreated, geo-referenced data due to a lack of data.
\item $Intercept = `r CAPFT1[[5]]`$ ($p_{Intercept} = `r CAPFT1[[6]]`$); $Slope = `r CAPFT1[[7]]`$ ($p_{Slope} = `r CAPFT1[[8]]`$) for mapped mean trait data
\end{enumerate}

\nmass is correlated with \ndvil memory effects as:
\begin{enumerate}
\item $Intercept = `r CAPFT1[[9]]`$ ($p_{Intercept} = `r CAPFT1[[10]]`$); $Slope = `r CAPFT1[[11]]`$ ($p_{Slope} = `r CAPFT1[[12]]`$) for untreated, geo-referenced data
\item $Intercept = `r CAPFT1[[13]]`$ ($p_{Intercept} = `r CAPFT1[[14]]`$); $Slope = `r CAPFT1[[15]]`$ ($p_{Slope} = `r CAPFT1[[16]]`$) for mapped mean trait data
\end{enumerate}

\clearpage

\paragraph*{Air Temperature}  \hspace{.5cm}
Relationships of \tair vegetation memory coefficients and \vh, and \nmass records across the Caatinga are represented in figure \ref{fig:CAPFT2}. These relationships have been assessed through linear regressions. 
```{r CAPFT2, echo = FALSE, fig.width = 17, fig.height = 5.5, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 1, out.width='1\\linewidth', fig.cap="\\textbf{PFTs and Tair Memory (Caatinga) -} Tair Memory (figure \\ref{fig:CAVegMem1}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:CaatTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:CAPFT2}", fig.scap="PFTs and Tair Memory (Caatinga)",  cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
CAPFT2 <- Tryres(Region = "Caatinga", SoilLayer = 1, Variable = "Tair")
```

Correlations of \vh with \tair memory effects are as follows:
\begin{enumerate}
\item Cannot be calculated for untreated, geo-referenced data due to a lack of data.
\item $Intercept = `r CAPFT2[[5]]`$ ($p_{Intercept} = `r CAPFT2[[6]]`$); $Slope = `r CAPFT2[[7]]`$ ($p_{Slope} = `r CAPFT2[[8]]`$) for mapped mean trait data
\end{enumerate}

\nmass is correlated with \tair memory effects as:
\begin{enumerate}
\item $Intercept = `r CAPFT2[[9]]`$ ($p_{Intercept} = `r CAPFT2[[10]]`$); $Slope = `r CAPFT2[[11]]`$ ($p_{Slope} = `r CAPFT2[[12]]`$) for untreated, geo-referenced data
\item $Intercept = `r CAPFT2[[13]]`$ ($p_{Intercept} = `r CAPFT2[[14]]`$); $Slope = `r CAPFT2[[15]]`$ ($p_{Slope} = `r CAPFT2[[16]]`$) for mapped mean trait data
\end{enumerate}

\clearpage

\paragraph*{Soil Moisture (0-7cm)}  \hspace{.5cm}
Relationships of \qsoila vegetation memory length, and coefficients and \vh, and \nmass records across the Caatinga are represented in figure \ref{fig:CAPFT3}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:CAPFTTab3}. 
```{r CAPFT3, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil1 Memory (Caatinga) -} \\qsoila Memory (figure \\ref{fig:CAVegMem1}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:CaatTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:CAPFT3}", fig.scap="PFTs and Qsoil1 Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
CAPFT3 <- Tryres(Region = "Caatinga", SoilLayer = 1, Variable = "Qsoil")
```
Linear regression identified statistically significant relationships for \qsoila related vegetation memory coefficients and species-specific mapped trait means of \vh and \nmass. All identified significant relationships are weak. No untreated, geo-referenced \vh data is available across the Caatinga.
...
\paragraph*{Soil Moisture (7-28cm)}  \hspace{.5cm}
Relationships of \qsoilb vegetation memory length, and coefficients and \vh, and \nmass records across the Caatinga are represented in figure \ref{fig:CAPFT4}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:CAPFTTab4}. 
```{r CAPFT4, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil2 Memory (Caatinga) -} \\qsoilb Memory (figure \\ref{fig:CAVegMem2}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:CaatTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:CAPFT4}", fig.scap="PFTs and Qsoil2 Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
CAPFT4 <- Tryres(Region = "Caatinga", SoilLayer = 2, Variable = "Qsoil")
```
Linear regression identified statistically significant relationships for \qsoilb related vegetation memory coefficients and species-specific mapped trait means of \vh and \nmass with the exception of \vh and \qsoilb memory effects. All identified significant relationships are weak. No untreated, geo-referenced \vh data is available across the Caatinga.
...
\paragraph*{Soil Moisture (28-100cm)}  \hspace{.5cm}
Relationships of \qsoilc vegetation memory length, and coefficients and \vh, and \nmass records across the Caatinga are represented in figure \ref{fig:CAPFT5}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:CAPFTTab5}. 
```{r CAPFT5, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil3 Memory (Caatinga) -} \\qsoilc Memory (figure \\ref{fig:CAVegMem3}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:CaatTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:CAPFT5}", fig.scap="PFTs and Qsoil3 Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
CAPFT5 <- Tryres(Region = "Caatinga", SoilLayer = 3, Variable = "Qsoil")
```
Linear regression identified statistically significant relationships for \qsoilc related vegetation memory coefficients and species-specific mapped trait means of \vh and \nmass with the exception of \nmass and \qsoilc memory length. All identified significant relationships are weak. No untreated, geo-referenced \vh data is available across the Caatinga.
...
\paragraph*{Soil Moisture (100-255cm)}  \hspace{.5cm}
Relationships of \qsoild vegetation memory length, and coefficients and \vh, and \nmass records across the Caatinga are represented in figure \ref{fig:CAPFT6}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:CAPFTTab6}. 
```{r CAPFT6, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil4 Memory (Caatinga) -} \\qsoild Memory (figure \\ref{fig:CAVegMem4}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:CaatTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:CAPFT6}", fig.scap="PFTs and Qsoil4 Memory (Caatinga)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
CAPFT6 <- Tryres(Region = "Caatinga", SoilLayer = 4, Variable = "Qsoil")
```
With the exception of mapped \vh and \qsoild memory effects as well as mapped \nmass and \qsoild memory length, linear regression analyses identified no statistically significant relationships for \qsoild related vegetation memory coefficients \vh and \nmass. All identified significant relationships are weak. No untreated, geo-referenced \vh data is available across the Caatinga.
...
#### Australia
\paragraph*{Intrinsic Memory}  \hspace{.5cm}
Relationships of \ndvil vegetation memory coefficients and \vh, and \nmass records across Australia are represented in figure \ref{fig:AUPFT1}. These relationships have been assessed through linear regressions. 
```{r AUPFT1, echo = FALSE, fig.width = 17, fig.height = 5.5, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 1, out.width='1\\linewidth', fig.cap="\\textbf{PFTs and Intrinsic Memory (Australia) -} Intrinsic Memory (NDVI t-1, figure \\ref{fig:AUVegMem1}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:AUTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:AUPFT1}", fig.scap="PFTs and Intrinsic Memory (Australia)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
\begin{enumerate}
\item Cannot be calculated for untreated, geo-referenced data due to a lack of data.
\item $Intercept = `r AUPFT1[[5]]`$ ($p_{Intercept} = `r AUPFT1[[6]]`$); $Slope = `r AUPFT1[[7]]`$ ($p_{Slope} = `r AUPFT1[[8]]`$) for mapped mean trait data
\end{enumerate}

\nmass is correlated with \ndvil memory effects as:
\begin{enumerate}
\item $Intercept = `r AUPFT1[[9]]`$ ($p_{Intercept} = `r AUPFT1[[10]]`$); $Slope = `r AUPFT1[[11]]`$ ($p_{Slope} = `r AUPFT1[[12]]`$) for untreated, geo-referenced data
\item $Intercept = `r AUPFT1[[13]]`$ ($p_{Intercept} = `r AUPFT1[[14]]`$); $Slope = `r AUPFT1[[15]]`$ ($p_{Slope} = `r AUPFT1[[16]]`$) for mapped mean trait data
\end{enumerate}

\clearpage

\paragraph*{Air Temperature}  \hspace{.5cm}
Relationships of \tair vegetation memory coefficients and \vh, and \nmass records across Australia are represented in figure \ref{fig:AUPFT2}. These relationships have been assessed through linear regressions. 
```{r AUPFT2, echo = FALSE, fig.width = 17, fig.height = 5.5, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 1, out.width='1\\linewidth', fig.cap="\\textbf{PFTs and Tair Memory (Australia) -} Tair Memory (figure \\ref{fig:AUVegMem1}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:AUTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:AUPFT2}", fig.scap="PFTs and Tair Memory (Australia)",  cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
\begin{enumerate}
\item Cannot be calculated for untreated, geo-referenced data due to a lack of data.
\item $Intercept = `r AUPFT2[[5]]`$ ($p_{Intercept} = `r AUPFT2[[6]]`$); $Slope = `r AUPFT2[[7]]`$ ($p_{Slope} = `r AUPFT2[[8]]`$) for mapped mean trait data
\end{enumerate}

\nmass is correlated with \tair memory effects as:
\begin{enumerate}
\item $Intercept = `r AUPFT2[[9]]`$ ($p_{Intercept} = `r AUPFT2[[10]]`$); $Slope = `r AUPFT2[[11]]`$ ($p_{Slope} = `r AUPFT2[[12]]`$) for untreated, geo-referenced data
\item $Intercept = `r AUPFT2[[13]]`$ ($p_{Intercept} = `r AUPFT2[[14]]`$); $Slope = `r AUPFT2[[15]]`$ ($p_{Slope} = `r AUPFT2[[16]]`$) for mapped mean trait data
\end{enumerate}

\clearpage

\paragraph*{Soil Moisture (0-7cm)}  \hspace{.5cm}
Relationships of \qsoila vegetation memory length, and coefficients and \vh, and \nmass records across Australia are represented in figure \ref{fig:AUPFT3}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:AUPFTTab3}.
```{r AUPFT3, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil1 Memory (Australia) -} \\qsoila Memory (figure \\ref{fig:AUVegMem1}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:AUTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:AUPFT3}", fig.scap="PFTs and Qsoil1 Memory (Australia)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
AUPFT3 <- Tryres(Region = "Australia", SoilLayer = 1, Variable = "Qsoil")
```
Linear regression identified statistically significant relationships for \qsoila related vegetation memory coefficients and species-specific mapped trait means of \vh and \nmass. All identified significant relationships are weak. No untreated, geo-referenced \vh data is available across Australia.
...
\paragraph*{Soil Moisture (7-28cm)}  \hspace{.5cm}
Relationships of \qsoilb vegetation memory length, and coefficients and \vh, and \nmass records across Australia are represented in figure \ref{fig:AUPFT4}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:AUPFTTab4}.
```{r AUPFT4, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil2 Memory (Australia) -} \\qsoilb Memory (figure \\ref{fig:AUVegMem2}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:AUTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:AUPFT4}", fig.scap="PFTs and Qsoil2 Memory (Australia)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
\paragraph*{Soil Moisture (28-100cm)}  \hspace{.5cm}
Relationships of \qsoilc vegetation memory length, and coefficients and \vh, and \nmass records across Australia are represented in figure \ref{fig:AUPFT5}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:AUPFTTab5}.
```{r AUPFT5, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil3 Memory (Australia) -} \\qsoilc Memory (figure \\ref{fig:AUVegMem3}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:AUTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:AUPFT5}", fig.scap="PFTs and Qsoil3 Memory (Australia)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
\paragraph*{Soil Moisture (100-255cm)}  \hspace{.5cm}
Relationships of \qsoild vegetation memory length, and coefficients and \vh, and \nmass records across Australia are represented in figure \ref{fig:AUPFT6}. These relationships have been assessed through linear regressions, the coefficients of which are depicted in table \ref{tab:AUPFTTab6}.
```{r AUPFT6, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{PFTs and Qsoil4 Memory (Australia) -} \\qsoild Memory (figure \\ref{fig:AUVegMem4}) plotted against expressions of (a) \\vh, and (b) \\nmass both via geo-referenced \\TRY records and mapped species-specific trait means (figure \\ref{fig:AUTRY}). Figure established via \\ref{rTRYRes}. \\label{fig:AUPFT6}", fig.scap="PFTs and Qsoil4 Memory (Australia)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
AUPFT6 <- Tryres(Region = "Australia", SoilLayer = 4, Variable = "Qsoil")
```
Linear regression identified statistically significant relationships for \qsoild related vegetation memory coefficients and species-specific mapped trait means of \vh and \nmass. All identified significant relationships - with the exception of \nmass and \qsoild memory length - are weak. No untreated, geo-referenced \vh data is available across Australia.
...
Linear regressions of \COMPADRE data against vegetation memory coefficients have proven **largely inconclusive**, especially due to low spatial \COMPADRE data coverage **across the Caatinga and Australia**.

Making use of the much higher spatial coverage of \COMPADRE data across the **Iberian region**, \COMPADRE \textbf{reactivity} emerged as an \textbf{important factor in explaining vegetation memory patterns}:
...
Analyses of the relationships of \PFT expressions and vegetation memory characteristics (summaries of which are provided in tables \ref{tab:IBPFTTabRaw}, \ref{tab:IBPFTTabMapped} , \ref{tab:CAPFTTabRaw}, \ref{tab:CAPFTTabMapped}, \ref{tab:AUPFTTabRaw}, and \ref{tab:AUPFTTabMapped}) revealed that:

\begin{enumerate}
\item \textbf{Geo-referenced \PFT records cannot be linked to vegetation memory characteristics} in a statistically significant way across all three study regions.
\item \textbf{Mapped species-specific \PFT means can be linked to patterns of vegetation memory} characteristics across all three study regions. However, these \textbf{relationships are weak}.
...
Overall, \textbf{\PFTs cannot be linked to vegetation memory characteristics in a meaningful way} via linear regressions.
...
## Vegetation Memory
\begin{itemize}
\item Memory effects sizes are ranked as follows: \ndvil $>$ \qsoil (\qsoila $>$ \qsoilb $>$ \qsoilc $>$ \qsoild) $>$ \tair \hfill \newline
$\rightarrow$ Intrinsic memory drives systems more than extrinsic memory \hfill \newline
$\rightarrow$ Research Question 1.1: Intrinsic metric is \ndvil, and extrinsic metric is \qsoil. However, this depends on the study region (we have only assessed dryland regions)
\item The higher the environmental mean of a certain vegetation memory driver, the lower/the more negative the vegetation memory effect \hfill \newline
$\rightarrow$ depicting plant niches/adaptive capabilites of vegetation
\item Overlap in patterns of \ndvil and \qsoila memory effects \hfill \newline
$\rightarrow$ suggesting that intrinsic memory might be a masked extrinsic memory effect \hfill \newline
$\rightarrow$ Variance partitioning shows that such a clear distinction is impossible with \ndvil variance being much bigger than \qsoil variance (especially in deeper soil layers) \hfill \newline
$\rightarrow$ Research Question 1.2: Due to the high portion of shared variance, I argue that it is diffuclt to distinguish intrinsic and extrinsic in certainr egions memory
\end{itemize}

### Intrinsic Memory
\begin{itemize}
\item Positive effects in dryland regions \hfill \newline
$\rightarrow$ Weaker engineering resilience across dryland regions
\item Negative correlation to effects of mean \ndvi records \hfill \newline
$\rightarrow$ high memory/low resilience linked to regions of low biomass (material legacies)
\end{itemize}
...
\begin{itemize}
\item strong/positive in drylands \hfill \newline
$\rightarrow$ dryland vegetation reacts positively to positive anomalies in soil moisture depicting the \textbf{water limitation} of drylands
\item \qsoila is the most informative \qsoil layer \newline
$\rightarrow$ vegetation on the three study regionspredimeninantly receives ater from the upper 7cm of the soil. \hfill \newline
$\rightarrow$ vegetation in the three study regions has adapted to quickly respond to precipitation events thus making it so that water is mostly taken up from the first soil layer befor it sinks to lower soil layers
\item Negative correlation of effects to mean \qsoil records \hfill \newline
$\rightarrow$ climate niche of plants (low resistance at upper and lower moisture spectrum, and high resistance to water anomalies at intermediate moisture levels)
\item Soil moisture vegetation memory sensitivity decreasing through layers \hfill \newline
$\rightarrow$ plants adapt the most to the soil moisture conditions of the upper 7cm of soil
\end{itemize}

\paragraph*{Soil Moisture Memory Length}  \hspace{.5cm}
\begin{itemize}
\item Shortest memory in regions of strong (positive or negative) memory effects \hfill \newline
$\rightarrow$ low resistance (memory strength) systems react fast (memory length) to perturbations
\item shorter memory in dryland regions overall \hfill \newline
$\rightarrow$ drylands react fast to \qsoil anomalies signifying the adaptation of plants to leverage soil moisture as quickly as possible
\item In regions of strong \qsoil memory, memory length is shortest in the Caatinga, intermediate across Australia and longest across the Iberian Region \hfill \newline
$\rightarrow$ impacts of \qsoil manifest on different timescales given geographical location and local factors which I haven't assessed here (further research?)
\item Negative correlation of memory length to mean \qsoil records \hfill \newline
$\rightarrow$ dry regions react fast (and positive) to \qsoil anomalies \hfill \newline
$\rightarrow$ wet regions react slow (and negative) to \qsoil anomalies
\item Soil moisture vegetation memory length sensitivity decreasing through layers \hfill \newline
$\rightarrow$ speed of vegetation response to \qsoil anomalies becomes decoupled from mean \qsoil conditions deeper in the soil identifying less adaptation of vegetation to the conditions in these layers
\item Soil moisture vegetation memory length sensitivity changing correlation sign through layers across the Iberian Region \hfill \newline
$\rightarrow$ Speed of vegetation response to \qsoil anomalies in dependence on mean \qsoil conditions turns around between \qsoilb and \qsoilc \hfill \newline
$\rightarrow$ This points to different root systems of different plants favouring different adaptations to \qsoil conditions
\end{itemize}

#### Air Temperature
\begin{itemize}
\item Negative in drylands \hfill \newline
$\rightarrow$ dryland vegetation reacts negatively to positive anomalies in \tair (instanteneous) depicting negative effects of increased evaporation and levels of aridity that are \tair-driven
\item Positive in northern regions and high altitudes \hfill \newline
$\rightarrow$ vegetation in cold/wet regions reacts positively to positive anomalies in \tair (instanteneous) depicting positive effects of increased solar radiation (coupled with higher \tair records) and increased speeds of biological processes in higher temperatures
\item Negative correlation of effects to mean \tair records \hfill \newline
$\rightarrow$ climate niche of plants (low resistance at upper and lower \tair spectrum, and high resistance to water anomalies at intermediate \tair levels)
\end{itemize}

## Functional Aspects of Vegetation Memory
\begin{itemize}
\item Research Question 2.1: \COMPADRE reactivity is related to vegetation memory \hfill \newline
$\rightarrow$ due to the direct link of irst-time-step response to recovery rates and resistance potential
...
```{r AUVegMem2, echo = FALSE, fig.width = 17, fig.height = 15, fig.subcap=c('SoilLag', 'Qsoil'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Vegetation Memory Coefficients (Australia; Qsoil2) -} Coefficients of vegetation memory obtained via model selection (figure \\ref{fig:ModScheme}) and \\pca regression (figure \\ref{fig:PCAREG}). For an interpretation of these coefficients, see table \\ref{tab:CoeffsOverview}. Figure established via \\ref{rModelRes}. \\label{fig:AUVegMem2}", fig.scap="Vegetation Memory Coefficients (Australia; Qsoil2)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
```{r AUVegMem4, echo = FALSE, fig.width = 17, fig.height = 15, fig.subcap=c('SoilLag', 'Qsoil'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Vegetation Memory Coefficients (Australia; Qsoil4) -} Coefficients of vegetation memory obtained via model selection (figure \\ref{fig:ModScheme}) and \\pca regression (figure \\ref{fig:PCAREG}). For an interpretation of these coefficients, see table \\ref{tab:CoeffsOverview}. Figure established via \\ref{rModelRes}. \\label{fig:AUVegMem4}", fig.scap="Vegetation Memory Coefficients (Australia; Qsoil4)", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
