...
PatcausRes <- function(Region, SoilLayer, SoilOnly){
  ...
  geom_boxplot() + theme_bw(base_size = 35) + xlab(paste("Mean Soil Moisture", QsoilTitle[SoilLayer])) + ylab(paste("Soil Moisture", QsoilTitle[SoilLayer], "Memory Length")) + coord_flip()
print(plotm)
...
Tryres <- function(Region, SoilLayer, Variable){
  ...
  data <- data.frame(Height = c(values(TryRaw[[1]]), values(TryDis[[1]])), 
  ...
  VarTitle <- c("NDVI [t-1] Coefficients", "Qsoil", "Air Tmperature Coefficients")[(LocVar-1)]
  if(LocVar == 3){
...
Comres <- function(Region, Variable){
  ...
  Variable <- VarHold
  plot_df <- na.omit(plot_df)
  leg <- ggplot(data = plot_df, aes(x = Compadre, y = Data, col = Identifiers)) + geom_point(alpha = 0.5, size = 3.5) + theme_bw(base_size = 35) +  xlab(paste("COMPADRE", Variable)) + ylab("Vegetation Memory Coefficients") + geom_hline(yintercept = 0, linetype="dotted") + stat_smooth(method = "lm", level = 0.66) + scale_color_manual(values=c("purple4", "purple2", "violetred2", "violet", "blue4", "blue2", "cyan4", "cyan2", "forestgreen", "red")) + guides(colour = guide_legend(ncol = 3, override.aes = list(size=20))) + labs(col='Memory Characteristics')
  plot <- ggplot(data = plot_df, aes(x = Compadre, y = Data, col = Identifiers)) + geom_point(alpha = 0.5, size = 3.5) + theme_bw(base_size = 35) +  xlab(paste("COMPADRE", Variable)) + ylab("Vegetation Memory Coefficients") + geom_hline(yintercept = 0, linetype="dotted") + stat_smooth(method = "lm", level = 0.66) + scale_color_manual(values=c("purple4", "purple2", "violetred2", "violet", "blue4", "blue2", "cyan4", "cyan2", "forestgreen", "red")) + theme(legend.position = "none")
  print(plot)
  return(leg)
}
```
...
# Introduction
Ecosystems are subject to disturbance regimes which can undergo substantial alterations\cite{Johnstone2016} causing bioclimatic challenges such as: (1) increased frequency and intensity of climate extremes\cite{Santiago2016}, (2) increasing temperatures and aridity\cite{Allen2010}, and (3) shifts in environemntal and ecosystem processes\cite{Kefi2016,Johnstone2016,Seidl2016b}. This has raised growing concern that future disturbance dynamics will exceed tipping points and cause non-linear shifts in ecosystem state in relation to **attractors**\cite{Senf2017} (see table \ref{tab:ResTerm} for definitions of bold font terms relating to the resilience debate). Such a **regime shift**\cite{Scheffer2010,Xu2015,Vasilakopoulos2015} can have a tremendous impact on local as well as global welfare of mankind\cite{Cote2010,Scheffer2001,Genkai-Kato2007}. Therefore, policy makers have made it their mission statement to enforce actions which serve to maintain or boost **resilience**: 

\begin{center}
\textit{"By 2020, ecosystem resilience [...] has been enhanced, [...] thereby contributing to climate change mitigation and adaptation and to combating desertification."}
\end{center}
\vspace{-.2cm}\flushright $\sim$ Aichi biodiversity target 15\cite{CBD2013}

\begin{center}
\textit{"We achieve climate resilience when short, medium and long-term decision making considers current climate risks and a changing climate."}
\end{center}
\vspace{-.2cm}\flushright $\sim$ Australian National Climate Resilience and Adaptation Strategy\cite{GovernmentDepartmentoftheEnvironment2015} \flushleft
...
Generally, resilience can be understood as the capacity of a system to absorb **disturbances** and respond to changing conditions so as to still retain the same function, structure, identity and feedbacks\cite{Vasilakopoulos2015,Holling1973}. First introduced to biological sciences in 1973 by Holling et al. resilience thinking was envisaged to encompass two separate characteristics of ecosystems\cite{Holling1973}:
...
Within this early context on resilience thinking, a system could be resilient whilst not being very stable and fluctuating in its properties a great deal\cite{Holling1973}.
...
Simultaneous assessment of components of any given ecosystem is a difficult task that often defies completion\cite{Standish2014}. This poses a **major research challenge** of choosing a measure by which to assess resilience and ecosystem components reliably through time and space. Consequently, recent studies have focussed on identifying and quantifying sub-components of ecosystem  resilience\cite{Seddon2016,Redmond2017,Papagiannopoulou2017,Gazol2017}. Hodgson et al. tackled the problem of resilience measurements by building a framework for resilience assessments by first representing resilience as two separate entities: (1) *Resistance* to perturbation and (2) *Recovery* from perturbation\cite{Hodgson2015}. 

Recovery rates have been used as proxies for resilience metrics in many study settings including (1) palaeoecologic research (e.g. by linking recovery rates of tropical ecosystems and disturbances categories\cite{Cole2014}), (2) observational studies (e.g. drought-impacts across different temporal and spatial scales\cite{Reyer2015}), and (3) experimental research (e.g. through applying small-scale disturbances and recording recovery rates\cite{VanBelzen2017a}).

Representing the *recovery* aspect of the Hodgson et al. framework to resilience is (amongst others) \textbf{ecological stress memory} (also know as 'ecological memory', see table \ref{tab:MemTerm} for definitions of bold font terms relating to the ecological memory framework). Ecological memory has been used to assess resilience of various different ecosystems. 

Nyström \& Folke reported a direct link between spatial resilience and ecological memory of coral reefs\cite{Nystrom2001}. In forest ecosystems, a positive link between ecological memory and ecosystem resilience has been identified by Johnstone et al\cite{Johnstone2016}.
...
Hence, ecosystem-scale studies have refocussed their attention on identifying components of ecological stress memory such as \textbf{vegetation memory}\cite{Walter2013,Johnstone2016}.  
...
Patterns and characteristics of vegetation memory are a vital information criterion for \dss aimed at conservation or management efforts due to the inverse relationship between vegetation memory and engineering resilience. However, one may also argue that vegetation memory indicates how well prepared a system is to face disturbances after experiencing similar perturbations. This issue is largely one of how vegetation memory is defined. Walter et al., for example, present an threefold understanding of vegetation responses built around memory\cite{Walter2013}:

\begin{enumerate}
\item \textit{Negative} response come about due to vegetation failing to adapt to stress conditions and suffering from decreased fitness upon a returning perturbation event.
\item \textit{Neutral} responses translate to plants being as prepared to face disturbances as they have been prior to other disturbances of the same kind.
\item \textit{Positive} responses are established through vegetation memory which is understood as an imprint on prior distrubance regimes and a bolstered resistance to these.
\end{enumerate}

For my work presented here, I will be regarding vegetation memory as the inverse of recovery rates (i.e. how long a perturbation effect persists).
...
Vicente-Serrano et al. used correlation coefficients to assess drought-related vegetation memory by calculating pearson correlation coefficients between monthly \ndvi and \spei records. This was done after aggregating biweekly \ndvi data to monthly maximum composites as well as standardisation. Strength and length of the identified memory effects can be seen in figure \ref{fig:Vincente}.

\begin{figure}[h!]
\centering \includegraphics[width=.8\linewidth]{3 - Figures/Vincente.png}
...
Liu et al. assessed vegetation memory in relation to water limitations globally. To do so, they built models of measures of \ndvi as predicted through \spi records at different temporal lengths, temperature records, and irradiation data. \cite{Liu2018}. Memory length was subsequently assessed as the lag of the \spi effect with the highest correlation coefficient within the resulting models. Before model building commenced, Liu et al. applied data preparation steps which removed areas and time steps from the analysis which did not meet certain criteria (i.e. a 5\% threshold on missing vegetation data, monthly \ndvi data below $0.1$, and monthly temperature below $0$ degrees Celsius). Subsequently, data was detrended and standardised for all variables prior to model building. Results of this study are presented in figure \ref{fig:Liu}.

\begin{figure}[h!]
\centering \includegraphics[width=.6\linewidth]{3 - Figures/Liu.png}
...
\begin{figure}[h!]
\centering \includegraphics[width=.8\linewidth]{3 - Figures/DeKeers.png}
...
\begin{enumerate}
\item \textit{High Memory Coefficients} indicating regions of low resilience can be found largely in \textit{dryland regions} (e.g.: North-East Brazil, Australia, and Spain)
\item \textit{Trends in Memory Length} signalling a change in causal patterns of memory effects can be found across the Iberian Peninsula, and the contiguous United States of America.
\end{enumerate}

Furthermore, all of these regions are characterised by gradients in vegetation memory intensity. Any study researching vegetation memory should focus on these regions of interest.
...
\PFTs convey information as *snapshots* of reality as their data sets usually do not contain repeated measures of the same individuals over a period of time. Another set of metrics related to plant characteristics through time are \textbf{\LHTs} (see table \ref{tab:LHTTerm} for definitions of bold font terms relating to \LHT frameworks). Despite recording expressions of plant function, \LHTs capture different information than \PFTs and should thus be regarded as *complementary* to \PFTs. Prominent examples of \LHTs include measures relating to (1) **System Turnover**, (2) **Longevity**, (3) **Growth**, and (4) **Reproduction**.
...
```{r IBRVegMem2, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('SoilLag', 'Qsoil'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
```{r IBRVegMem3, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('SoilLag', 'Qsoil'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
```{r IBRVegMem4, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('SoilLag', 'Qsoil'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
...
```{r IBPFT1, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
Tryres(Region = "Iberian Region", SoilLayer = 1, Variable = "t-1")
```

```{r IBPFT2, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
Tryres(Region = "Iberian Region", SoilLayer = 1, Variable = "Tair")
```

\clearpage

```{r IBPFT3, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
Tryres(Region = "Iberian Region", SoilLayer = 1, Variable = "Qsoil")
```

```{r IBPFT4, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
Tryres(Region = "Iberian Region", SoilLayer = 2, Variable = "Qsoil")
```

\clearpage

```{r IBPFT5, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
Tryres(Region = "Iberian Region", SoilLayer = 3, Variable = "Qsoil")
```

```{r IBPFT6, echo = FALSE, fig.width = 17, fig.height = 14, fig.subcap=c('Height', 'Nitrogen'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
Tryres(Region = "Iberian Region", SoilLayer = 4, Variable = "Qsoil")
```
...
```{r IBCOMPAD, echo = FALSE, fig.width = 17, fig.height = 12, fig.subcap=c('FSC-1', 'FSC-2', 'Pi', 'Rho', 'Reactivity', 'Plotting Colour Reference'), fig.ncol = 2, out.width='.5\\linewidth', fig.cap="\\textbf{Short -} Long. \\label{fig:Dummy}", fig.scap="Short", cache = TRUE, cache.lazy=FALSE, fig.pos = 'h!'}
leg <- Comres(Region = "Iberian Region", Variable = "FSC-1")
leg <- Comres(Region = "Iberian Region", Variable = "FSC-2")
leg <- Comres(Region = "Iberian Region", Variable = "Pi")
leg <- Comres(Region = "Iberian Region", Variable = "Rho")
leg <- Comres(Region = "Iberian Region", Variable = "Reactivity")
legend <- cowplot::get_legend(leg)
grid.newpage()
grid.draw(legend)
```
