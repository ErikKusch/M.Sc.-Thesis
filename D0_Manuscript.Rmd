...
\begin{abstract}
The dependence of humans on stable, intact ecosystems, whose future characteristics and responses to perturbation are understood, has lead to an increasing appreciation of the need for resilience goals to be incorporated into environmental policy catalogues\cite{CBD2013,Newton2016}. Contemporary ecology recongnises vegetation memory as an important proxy to ecosystem resistance and recovery - two main components of resilience\cite{Hodgson2015}. Previous research of vegetation has identified ecologically relevant patterns of vegetation memory effects across dryland regions\cite{DeKeersmaecker2015,Liu2018,Vicente-Serrano2013} coinciding with vegetation sensitivity towards climatological drivers\cite{Seddon2016}.

My study presents these studies in a synthesis with the distinction of intrinsic and extrinsic vegetation memory components proposed by Ogle et al\cite{Ogle2015}. Additionally, I am introducing the use of state-of-the-art climate reanalysis data into refined approaches to identifying vegetation memory characteristics across dryland regions. Thas has lead to novel insights into spatial patterns of vegetation memory characteristics across three distinct predominantly dryland regions (Iberian Region, Caatinga, and Australia).

Finally, my thesis leverages the potential explanatory power contained within large data bases of functional ecology (i.e. TRY and COMPADRE). This is done to establish connections between plant life history strategies and vegetation memory characteristics thereby identifying causal pathways enabling vegetation memory. I have identified biological first time-step response of populations towards perturbations as well as plant reproductive startegy to be related to vegetation memory capabilites.

Thus, my study presents a synthesis of resilience thinking, vegetation memory modelling approaches, climate science, and functional ecology. My findings demonstrate novel observations of vegetation memory patterns across dryland regions such as regional differences of processes forming vegetation memory capabilites. Consequently, this study may be a helpful stepping stone for refining and combining already existing methodology which could, in turn, generate important knowledge of ecosystem functioning and resilience.
\end{abstract}
...
\begin{enumerate}
\item \textit{High Response Coefficients} indicating regions of low resilience can be found largely in \textit{dryland regions} (e.g.: North-East Brazil, Australia, and Spain; figures \ref{fig:Vincente} and \ref{fig:DeKeers}).
...
My study uses \textit{\qsoil} as a proxy of local water regimes. I suggest that water-dependent vegetation memory largely relies on water availabilit to root systems. Precipitation events are subject to further soil processes such as pore connectivity for precipitation water to be available to plant roots\cite{Smith2017}. Soil moisture should thus serve as a much more direct proxy of local water regimes. Additionally, \era includes four distinct layers in the soil for the calculation of \qsoil indices (see figure \ref{fig:QsoilOverview} for an overview): (1) \qsoila, (2) \qsoilb, (3) \qsoilc, and (4) \qsoild. Typical drought indices (i.e. \spei) do not allow for this additional distinction. Within \era, unfrozen ground water ($\theta$) across all four soil layers ($k$) is defined as:
\vspace{-.2cm}
\begin{equation}
...
Due to the focus on dryland regions, the \textbf{main focus of my work is on \qsoil effects}. Following the vegetation sensitivity index by Seddon et al.\cite{Seddon2016}, these regions can easily be postulated to be heavily influenced by water availability regimes and lesser so by \tair regimes.
...
\begin{enumerate}
  \item \textit{Vegetation Response Coefficients} to identify intrinsic/extrinsic forcing factors answering \textit{research question I.1}. 
...
\ndvil memory is stronger than \qsoila memory which, in turn, is stronger than \tair vegetation responses (see table \ref{tab:IBRU1}).
...
```
\vspace{-.1cm}
Regression slopes of \qsoila memory coefficients and mean \qsoila reported in table \ref{tab:PatCauseTabQsoil1}vary widely and support the patterns reported in figure \ref{fig:PatCauseQsoil1}. Despite the disparity of regressions slopes, the rgeression intercepts are somewhat similar suggesting a similar baseline \qsoila vegetation response in the dryest regions.
\vspace{-.1cm}
...
\begin{itemize}
\item As \qsoila memory length increases (i.e. vegetation reacts slower), \qsoila memory itensifies across the Iberian Region and Australia. This effect is much stronger across the Iberian Region, whereas the effect size across Australia is much smaller and very close to zero.
...
Linking \fsca (life history speed) records to vegetation response coefficients has proven difficult. See figure \ref{fig:IBCOMPAD1} for a visual representation. Whilst most intercepts of these regressions are statistically significant, only the regression slope of \tair vegetation response coefficients and \fsca is statistically significant but at a small effect size (see table \ref{tab:IBCOMPADTab1}). This suggest a weak link between \tair response coefficients and \fsca records with faster life hsitories leading to larger vegetation responses to \tair anomalies.

```{r IBCOMPADTab1, results = 'asis', echo = FALSE}
CompadTab(Region = "Iberian Region", ModelCoeffs = c(unlist(leg1[[2]])), Variable = "FSC-1")
```

Linear relationships of \fscb (reproductive strategy) and vegetation response coefficients across the Iberian region (see figure \ref{fig:IBCOMPAD2}) identified all intercepts (see table \ref{tab:IBCOMPADTab2}) to be statistically significant, only the regression slopes of \ndvil and \tair vegetation response coefficients and \fscb. \ndvil memory decreases with increasing \fscb records. \tair vegetation responses increases with increasing \fscb records.
...
# Discussion
The first research goal of my thesis was to identify and map vegetation memory characteristics across dryland regions. Additionally, I was aiming to analyse how these patterns may come about (i.e. second research goal).
...
\item \qsoil memory and \qsoil memory length are linked differently across the three study regions. Whilst short \qsoil vegetation memory overlaps with strong \qsoil response coefficients across the Caatinga, the reverse is true considering the Iberian Region and Australia. I conclude that vegetation in the Caatinga region racts almost instanteneously to \qsoil anomalies whereas the vegetation of the Iberian Region exerts strongest \qsoil memory at long time scales. Australian vegetation exhibits a clear west-to-east pattern of decreasing \qsoil vegetation memory length.
\end{enumerate}
...
This is especially true going forward. As discussed above, I propose for further studies of vegetation memory to implement additional relevant bioclimatic variables at finer temporal resolutions and longer time scales. The \era data set supplies all that and more:
\begin{enumerate}
...
\item \textbf{Reactivity} - First time-step biological response to perturbations.
    \begin{itemize}
      ...
      \item \textbf{Higher reactivity} prompts \textbf{increased \tair vegetation response intensity} singalling decreasing instanteneous resistance to \tair anomalies in system of higher reactivity. This is in contrast to the \qsoil resistance and I argue that there may be two reasons for this: (1) increased reactivity comes at the cost of \tair resistance, or (2) instanteneous response can't be captured by first-time-step responses.
    \end{itemize}
\end{enumerate}
