...
col.list <- list(col.varpar1, col.varpar2, col.varpar3) 
  ...
  if(Legend == TRUE){
   p <- ggplot(data = plot_df, aes(y = Data, x = Cell, fill = Variance)) + geom_bar(stat = "identity") + ylim(0, quantile(values(Alter_ras[[1]]),.95, na.rm=TRUE)) + theme_bw(base_size= 20) + xlab("Raster Cells") + ylab("Variance") + scale_fill_manual(values=c(col.list[[3]][1], col.list[[2]][50], col.list[[1]][30])) 
  }else{
   p <- ggplot(data = plot_df, aes(y = Data, x = Cell, fill = Variance)) + geom_bar(stat = "identity") + ylim(0, quantile(values(Alter_ras[[1]]),.95, na.rm=TRUE)) + theme_bw(base_size= 20) + xlab("Raster Cells") + ylab("Variance") + scale_fill_manual(values=c(col.list[[3]][1], col.list[[2]][50], col.list[[1]][30])) + theme(legend.position = "none")
  }
  if(Plot == 1){
    return(p)
  }else{
...
\item \ndvil memory follows the same pattern as presented by De Keersmaecker et al.\cite{DeKeersmaecker2015} with strong, positive memory effects in the south of the Iberian region which diminish but remain positive towards the north. Across Spain, the \ndvil pattern is a close match of the \qsoila memory length pattern.
\item \qsoila memory is predominantly positive in sign (with a notable exception of the Pyrenees). Especially strong \qsoila memory can be observed in the dryland regions of Spain and Portugal. \qsoila emory patterns closely resemle \ndvil memory patterns.
...
### Variance Partitioning
Whilst the contents of tables \ref{tab:IBRU1}, \ref{tab:CAU1}, and \ref{tab:AUU1} allow for region-wide comparisons of relative importance of model components, variance partitioning allows for far more advanced spatial analyses of relative model coefficient importance. Spatial patterns of variance partitioning results are contained in figures (1) \ref{fig:IBRVarPar1} and \ref{fig:IBRVarPar2} for the Iberian Region, (2) \ref{fig:CAVarPar1} and \ref{fig:CAVarPar2} for the Caatinga, and (3) \ref{fig:AUVarPar1} and \ref{fig:AUVarPar2} for Australia.

Firstly, variance partitioning highlights the overall importance of \qsoila when compared to the variance explained by \qsoilb, \qsoilc, and \qsoild. Secondly, comparing variance partitioning results across study regions (see figure \ref{fig:VarParOverview}) patterns of different ecological system processes emerge. Whilst \qsoil and \ndvil share little to no variance across the Iberian Region, a moderate amount of variance is shared by these two variables when analysing the Caatinga range. Across Australia, however, large amounts of variance are shared by \ndvil and \qsoila. These changes hint at differences in how local vegetation is influenced by soil moisutre in these regions. Overall, \ndvil explains more variation in records of \ndvi than \qsoila.
...
```
Overall, values of \tair memory effects decrease as mean \tair records increase. This relationship is most pronounced across the Iberian Region and least pintense across Australia. This may be due to the presence of altitudinal gradients across the Iberian Region which are largely basent from Australia thus limitting the range of mean \tair values. On the other hand, this may be indicative of different patterns of vegetation adaptation to \tair conditions. Take note that most \tair memory effects are negative across the selected dryland study regions.
...
### Vegetation Memory Models
#### Iberian Region
\paragraph*{Model Coefficients}  \hspace{.5cm}
