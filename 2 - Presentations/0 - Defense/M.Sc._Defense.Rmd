...
\begincols[T]
  ...
  \begin{enumerate}
      ...
      \item Expressions of \textit{Plant Functional} as a possible solution
    \end{enumerate}
    \pause
    \vspace{0.3cm}
    \begin{tcolorbox}[colback=bondiblue!5,colframe=bondiblue!40!black,title= ]
...
### Why do this?
\centering \textbf{Vegetation Memory} proposed to be a \textit{proxy} of \textbf{engineering resilience in ecosystems} (\textit{high memory} $\sim$ \textit{low resilience})\cite{DeKeersmaecker2015}.
...
\includegraphics[width=1\linewidth]{Figures/Scheme_VegMem.jpg}
...
## Climate Data
### ERA5 & Variables I
\centering We are using the \textbf{ERA5} Data Set\cite{Albergel2013,Balsamo2015}.  
...
\item \textbf{Why:}
  ...
  \item More sophisticated approach than previously utilised:
    ...
    \end{itemize}
  \item \textit{Climatological Relevance:} Assesment of climate uncertainty possible
  \end{itemize}
\pause
\vspace{.2cm}
\item \textbf{Core Characteristics:}
  \begin{itemize}
  \item \textit{Resolution:} $30km \times 30km$
  \item \textit{Availability:} 1950 - Present
  \end{itemize}
\end{itemize}

### ERA5 & Variables II
We are using the following \textbf{climate variables}:
\begin{itemize}
\vspace{.2cm}
\visible<2->{
\item \textbf{Soil Moisture} - $Qsoil$
  ...
  \begin{itemize}
  \visible<3->{
  \item \textit{Why:} Soil moisture effects on vegetation may indiciate if/how the local ecosystem may deal with drought stress\cite{Rudgers2018,Smith2017,Papagiannopoulou2017a}.
  \item \textit{How:} as different layers of depth
    \begin{itemize}
    \item $Qsoil$ - 0-7cm layer
    \item $Qsoil2$ - 7-28cm layer
    \item $Qsoil3$ - 28-100cm layer
    \item $Qsoil4$ - 100-255cm layer
    \end{itemize}
  }
  \end{itemize}
\vspace{.2cm}
...
\begin{tcolorbox}[colback=green!5,colframe=green!40!black,title= ]
\centering Some LHTs can be linked to some vegetation memory characteristcs,
\end{tcolorbox}
